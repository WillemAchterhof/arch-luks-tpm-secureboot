...
EOF
    ...
    # Deploy WiFi connection profile for NetworkManager
    if [[ -n "$active_ssid" && -n "$active_pass" ]]; then
        local wifi_uuid nm_conn_dir wifi_template
        wifi_uuid=$(cat /proc/sys/kernel/random/uuid)
        nm_conn_dir="$mnt/etc/NetworkManager/system-connections"
        wifi_template="$CONFIGS_DIR/system/wifi.nmconnection"

        [[ -f "$wifi_template" ]]             || fatal "Missing config: $wifi_template"

        mkdir -p "$nm_conn_dir"

        sed             -e "s/WIFI_SSID/${active_ssid}/g"             -e "s/WIFI_PSK/${active_pass}/"             -e "s/WIFI_UUID/${wifi_uuid}/"             "$wifi_template"             > "$nm_conn_dir/${active_ssid}.nmconnection"

        chmod 600 "$nm_conn_dir/${active_ssid}.nmconnection"
        log "[*] WiFi connection profile deployed for: $active_ssid"
    else
        log "[!] No WiFi credentials — skipping NetworkManager profile."
    fi
